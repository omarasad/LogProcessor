...
public class AdvancedStrategyDynamicReqClassRep extends
		RequestBasedAnalyserGraphStrategyRep {

	public AdvancedStrategyDynamicReqClassRep(RequestBasedAnalyserGraphRep analyser) {
		super(analyser);
	}
...
public void analyzeUrlPartition(String url, ASServer serv) {	
	...
	public void tempVerify()
	{
		
		
		for (ASServer as : manager.servers.values())
		{
			Analyser.log.info(" ########################## server=" +as.serverId);
			int err=0;			   
			Iterator<String> ir=as.curReqListString.iterator();
			while (ir.hasNext())
			{
				
				String req=ir.next();
				Analyser.log.info("" +req);
				 HashSet<String> objects = manager.globalRequestToObjectMap.get(req);
				 Iterator<String> obj=objects.iterator();
				 if (!as.curObjList.contains(obj.next()))
					 err++;
			...
			Analyser.log.info(" server=" +as.serverId +"err="+err);
	...
	private void doAssignAllObjFirstTempReplicateAll()
	{

		Iterator<String> ir=manager.globalCacheMap.keySet().iterator();
		
		while (ir.hasNext())
		{
			String CacheString=  ir.next();
			CacheObject co=manager.globalCacheMap.get(CacheString);
			for (ASServer as : manager.servers.values())
			{
				
				as.curObjList.add(CacheString);
				co.sites.add(as.serverId);
				cacheObjAllocated[as.serverNo]++;
			}
			co.replicate=true;
	...
	private void doAssignAllObjFirstTemp()
	{
		...
		Analyser.log.info("No. of object to replicate="+tempNoSites);
	}
	...
	public boolean generatePolicies() throws Exception {
		...
		// This is normal dist and Rep
		doAssignAllObjFirstTemp();
		
		
		//doAssignAllObjFirstTempReplicateAll();
