...
public class RequestBasedAnalyserGraphMetis extends Analyser {
	...
	RequestBasedAnalyserGraphMetisStrategy strategy = null;
	...
	public RequestBasedAnalyserGraphMetis currentAnalysisPhase;
	...
	//		continue;
		...
		if (requests.size()==1)
		{
			...
			String onlyReq= ir.next();
			
			if (!ConsiderReq(onlyReq))
			{
				//Analyser.log.info("ConsiderReq(s1) continue ="+s1);
				continue;
			}
			
			if (onlyReq.contains("-1") || (onlyReq==null)  || onlyReq.contains("RegisterItem"))
				continue;
			
			requestsGraph.addVertex(onlyReq);
			requestsGraph.addEdge(onlyReq,onlyReq);
			*/
...
public void generateVertexIdsForNonInc()
{
	...
	public RequestBasedAnalyserGraphMetis(StatisticsManager manager) {
		super(manager);
		setupStrategy();
	...
	public void setupStrategy(){
		...
		strategy = (RequestBasedAnalyserGraphMetisStrategy) c.getConstructor(new Class[] { RequestBasedAnalyserGraphMetis.class }).newInstance(this);
...
return (HashMap<String, HttpRequestObject>) manager.globalRequestMap.clone();	
	...
	public void analyse()throws Exception {
		...
		this.currentAnalysisPhase= new RequestBasedAnalyserGraphMetis(manager);
			...
			{		
				...
				//	System.out.println(serv.currentPolicy);
					...
					*/
					...
					{
						System.out.println(serv.serverId +":" + es.getKey().toString());
					}
